...
const prototype = (
  ...
  path,
  component: () => import("../views/PrototypeModuleView.vue"),
  meta: { title, group, prototype: key, anyPermissions },
});
const children: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    meta: { title: "高级检索", group: "01 数据驾驶舱" },
  },
  ...
  ),
  {
    ...
    meta: { title: "机器学习", group: "04 研发数据中心" },
  },
  prototype(
  ...
  prototype(
    "quality/ct-metallography",
    "ct-metallography",
    "CT / 金相",
    "06 质量数据中心",
  ),
  prototype(
    "quality/hardness-thickness",
    "hardness-thickness",
    "硬度 / 厚度",
    "06 质量数据中心",
  ),
  ...
  ),
  {
    ...
    meta: {
      title: "外部系统",
      group: "08 数据集成中心",
      anyPermissions: ["integration:read", "integration:manage"],
    },
  },
  {
  ...
  },
  {
    path: "system/sharing",
    component: () => import("../views/AdminView.vue"),
    meta: { title: "共享管理", group: "10 系统管理", permission: "share:read" },
  },
  {
    path: "system/audits",
    component: () => import("../views/AuditsView.vue"),
    meta: { title: "审计日志", group: "10 系统管理", permission: "audit:read" },
  },
...

  {
    path: "templates",
    component: () => import("../views/TemplatesView.vue"),
    meta: { title: "模板中心", permission: "template:read" },
  },
  {
    path: "datasets",
    component: () => import("../views/DatasetsView.vue"),
  ...
  {
    path: "datasets/:id",
    component: () => import("../views/DatasetRecordsView.vue"),
    meta: { title: "数据记录", permission: "dataset:read" },
  },
  {
    path: "trace",
    component: () => import("../views/TraceView.vue"),
    meta: { title: "全链路追溯", permission: "trace:read" },
  },
  {
    path: "devices",
    component: () => import("../views/DevicesView.vue"),
    meta: { title: "设备与采集", permission: "device:read" },
  },
  {
    path: "files",
    component: () => import("../views/FilesView.vue"),
    meta: { title: "附件中心", permission: "file:read" },
  },
  {
    path: "lifecycle",
    component: () => import("../views/LifecycleView.vue"),
    meta: { title: "数据生命周期", permission: "lifecycle:read" },
  },
  {
    path: "integrations",
    component: () => import("../views/IntegrationsView.vue"),
  ...
  },
  {
    ...
    meta: {
      title: "系统与共享管理",
      anyPermissions: ["user:manage", "share:read"],
    },
  },
  {
    path: "governance",
    component: () => import("../views/GovernanceView.vue"),
    meta: { title: "治理配置", permission: "governance:read" },
  },
  {
    path: "profile",
...
function landingPath(auth: ReturnType<typeof useAuthStore>) {
  const candidates = [
    ...
    ["dataset:read", "/assets/overview"],
    ["trace:read", "/trace/search"],
    ["device:read", "/assets/devices"],
    ["integration:read", "/integration/overview"],
    ["integration:manage", "/integration/overview"],
    ["governance:read", "/governance/master-data"],
    ["user:manage", "/system/users"],
    ["share:read", "/system/sharing"],
  ];
  return (
    candidates.find(([permission]) => auth.can(permission))?.[1] ||
    "/research/projects"
  );
}
router.beforeEach(async (to) => {
  ...
  if ((to.path === "/login" || to.path === "/register") && auth.authenticated)
  ...
  if (
    Array.isArray(to.meta.anyPermissions) &&
    !to.meta.anyPermissions.some((permission) => auth.can(String(permission)))
  )
    return landingPath(auth);
});
