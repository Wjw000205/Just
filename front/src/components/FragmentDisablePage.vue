<template>
  <section class="fragment-disable-page">
    <!-- 面包屑导航 -->
    <div class="disable-header">
      <div class="disable-breadcrumb">
        ...
        <span class="crumb-now">模板片段停用</span>
      </div>
    </div>

    <div class="disable-content">
      <div class="main-title">模板片段停用</div>
      ...
      <div class="search-area">
        ...
        <div class="search-item">
          <label class="search-label">模板片段说明</label>
          <input
        ...
        <div class="search-item">
          ...
          <div class="search-select" @click.stop="toggleStatusDropdown">
            ...
            <ul v-if="statusDropdownVisible" class="search-select-dropdown">
              <li class="search-select-item" @click.stop="selectStatus('')">请选择模板状态</li>
              <li class="search-select-item" @click.stop="selectStatus('正常')">正常</li>
              <li class="search-select-item" @click.stop="selectStatus('不可使用')">不可使用</li>
            </ul>
          </div>
        </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th class="col-desc">模板片段说明</th>
              <th class="col-creator">创建人</th>
              <th class="col-create-time">创建时间</th>
              <th class="col-status">停用状态</th>
              <th class="col-disable-user">停用人</th>
              <th class="col-disable-time">停用时间</th>
          ...
          <tbody>
            <tr v-for="(item, index) in tableData" :key="item.id">
              ...
              <td class="col-status">
                <span class="status-badge" :class="item.status === '正常' ? 'normal' : 'disabled'">
                  {{ item.status }}
                </span>
              </td>
              <td class="col-disable-user">{{ item.disableUser || '-' }}</td>
              <td class="col-disable-time">{{ item.disableTime || '-' }}</td>
              <td class="col-action">
...
// 模拟表格数据
const tableData = ref([
  { id: 1, name: '表征结果模板', description: '', creator: '朱向东', createTime: '2026-03-11 21:41:51', status: '正常', disableUser: '', disableTime: '', selected: false },
  { id: 2, name: '表征模板', description: '', creator: '朱向东', createTime: '2026-03-11 21:39:51', status: '正常', disableUser: '', disableTime: '', selected: false },
  { id: 3, name: '设备名称/设备厂家/设备型号', description: '', creator: '朱向东', createTime: '2026-03-08 22:07:03', status: '正常', disableUser: '', disableTime: '', selected: false },
  { id: 4, name: '材料基本信息', description: '', creator: '朱向东', createTime: '2026-01-26 07:22:07', status: '正常', disableUser: '', disableTime: '', selected: false },
  { id: 5, name: '单轴压缩测试', description: '', creator: '朱向东', createTime: '2026-01-23 11:57:17', status: '正常', disableUser: '', disableTime: '', selected: false },
  { id: 6, name: '单轴压缩', description: '', creator: '朱向东', createTime: '2026-01-23 11:53:32', status: '不可使用', disableUser: '', disableTime: '', selected: false }
])
...
const handleDisable = (item) => {
  if (confirm(`确定要停用模板片段 "${item.name}" 吗？`)) {
    item.status = '不可使用'
    item.disableUser = '当前用户'
    item.disableTime = new Date().toLocaleString('zh-CN')
  }
}
